...
public final class BibProceedingsBuilder extends BibBookRecordBuilder {
   ...
   *          the location
   */
  public synchronized final void setLocation(
      final BibOrganization location) {
    this.fsmFlagsAssertAndUpdate(FSM.FLAG_NOTHING,
  ...
  /** {@inheritDoc} */
  @Override
  final void _handleOrganization(final BibOrganization org,
      final int tag) {
    if (tag == 1) {
  ...
  final BibProceedings _doCompile() {
    ...
    return new BibProceedings(true, this.m_title, this.m_date,
        ...
        this.m_series, this.m_issn, this.m_volume, this.m_isbn, this.m_url,
        this.m_doi);
  }
