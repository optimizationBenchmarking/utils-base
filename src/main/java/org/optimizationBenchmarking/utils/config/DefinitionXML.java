...
public enum DefinitionXML implements IXMLFileType {
  ...
  URI.create(//
      "http://www.optimizationBenchmarking.org/formats/configuration/configurationDef.1.0.xsd") //$NON-NLS-1$
      .normalize();
  ...
  public final URL getSchemaSource() throws IOException {
    ...
    u = this.getClass().getResource(//
        DefinitionXML.NAMESPACE
            .substring(DefinitionXML.NAMESPACE.lastIndexOf('/') + 1));
    if (u != null) {
      return u;
    }
