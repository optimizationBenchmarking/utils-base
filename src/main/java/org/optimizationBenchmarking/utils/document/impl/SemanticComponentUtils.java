...
public final class SemanticComponentUtils {
   ...
   * the method
   * {@link org.optimizationBenchmarking.utils.document.spec.IMathRenderable#mathRender(ITextOutput, IParameterRenderer)}
   * and it delegates to
   * {@link org.optimizationBenchmarking.utils.document.spec.IMathRenderable#mathRender(ITextOutput, IParameterRenderer)}
   * .
   *
  ...
  public static final void mathRender(final String name,
    ...
    if (out instanceof IComplexText) {
      try (final IMath math = ((IComplexText) out).inlineMath()) {
        SemanticComponentUtils.mathRender(name, math, renderer);
      }
    } else {
      out.append(name);
    }
   ...
   * {@link org.optimizationBenchmarking.utils.document.spec.IMathRenderable#mathRender(IMath, IParameterRenderer)}
   * and it delegates to
   * {@link org.optimizationBenchmarking.utils.document.spec.IMathRenderable#mathRender(IMath, IParameterRenderer)}
   * .
   *
