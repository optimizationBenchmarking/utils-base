...
public enum EFigureSize {
   ...
   */
  private static final double GOLDEN_RATIO = 0.61803398874989484820458683436564d;
   ...
   */
  public final PhysicalDimension approximateSize(
      final PageDimension page) {
    final ELength pageUnit;
    final double availableWidth, availableHeight, minExtend, minWidth,
        minHeight;
    double width, height;
    ...
    availableWidth = pageUnit
        .convertToAsDouble((this.m_spansAllColumns ? page.getWidth()//
            : page.getColumnWidth()), ELength.POINT);
    hasAvailableWidth = ((availableWidth > 0d)
    ...
    availableHeight = pageUnit.convertToAsDouble(page.getHeight(),
        ELength.POINT);
    hasAvailableHeight = ((availableHeight > 0d)
    ...
    if (!hasWidth) {
      if (hasHeight) {
        ...
        width = EFigureSize.__round(height / EFigureSize.GOLDEN_RATIO);
      } else {
    ...
    if (!hasHeight) {
      if (hasWidth) {
        ...
        height = EFigureSize.__round(width * EFigureSize.GOLDEN_RATIO);
      } else {
