 ...
 * bibliographic data. The idea of our hierarchical API &ndash; discussed
 * <a href="http://blog.it-weise.de/p/347">here</a> &ndash; is that we are
 ...
 * {@link #afterChildOpened(HierarchicalFSM, boolean)} method is invoked.
 * </li>
...
public class HierarchicalFSM extends FSM implements IScope {
  ...
  private static final int STATE_OPEN_BEGIN = (HierarchicalFSM.STATE_NEW
      + 1);

  /** we begin opening this element as a child element */
  private static final int STATE_BEFORE_BEFORE_CHILD_OPENS_CHILD_A = (HierarchicalFSM.STATE_OPEN_BEGIN
      + 1);

  /** we begin opening this element as a child element */
  private static final int STATE_BEFORE_BEFORE_CHILD_OPENS_CHILD_B = (HierarchicalFSM.STATE_BEFORE_BEFORE_CHILD_OPENS_CHILD_A
      + 1);

  /** the afterChildOpen method of the owner has been called */
  private static final int STATE_AFTER_BEFORE_CHILD_OPENS_CHILD = (HierarchicalFSM.STATE_BEFORE_BEFORE_CHILD_OPENS_CHILD_B
      + 1);

  /** the {@link #onOpen()} method has been called */
  private static final int STATE_AFTER_ON_OPEN = (HierarchicalFSM.STATE_AFTER_BEFORE_CHILD_OPENS_CHILD
      + 1);
  /**
   * the {@link #afterChildOpened(HierarchicalFSM,boolean)} method of the
   * owner has been called
   */
  private static final int STATE_AFTER_AFTER_CHILD_OPENED_CHILD = (HierarchicalFSM.STATE_AFTER_ON_OPEN
      + 1);
   ...
   * @see #open()
   */
  private static final int STATE_OPENED = (HierarchicalFSM.STATE_AFTER_AFTER_CHILD_OPENED_CHILD
      + 1);

  /** the before child open call has been done */
  private static final int STATE_BEFORE_BEFORE_CHILD_OPENS_OWNER = (HierarchicalFSM.STATE_OPENED
      + 1);
  /** we are after the before child open call */
  private static final int STATE_AFTER_BEFORE_CHILD_OPENS_OWNER_A = (HierarchicalFSM.STATE_BEFORE_BEFORE_CHILD_OPENS_OWNER
      + 1);
  /** we are after the before child open call */
  private static final int STATE_AFTER_BEFORE_CHILD_OPENS_OWNER_B = (HierarchicalFSM.STATE_AFTER_BEFORE_CHILD_OPENS_OWNER_A
      + 1);
  /** we begin the after child open call */
  private static final int STATE_BEFORE_AFTER_CHILD_OPENED_OWNER = (HierarchicalFSM.STATE_AFTER_BEFORE_CHILD_OPENS_OWNER_B
      + 1);
  /** the after child open call has been done */
  private static final int STATE_AFTER_AFTER_CHILD_OPENED_OWNER = (HierarchicalFSM.STATE_BEFORE_AFTER_CHILD_OPENED_OWNER
      + 1);

  /** the state right before the onChildClosed method is called */
  private static final int STATE_BEFORE_AFTER_CHILD_CLOSED_OWNER = (HierarchicalFSM.STATE_AFTER_AFTER_CHILD_OPENED_OWNER
      + 1);
  /** the state right after the onChildClosed method is called */
  private static final int STATE_AFTER_AFTER_CHILD_CLOSED_OWNER = (HierarchicalFSM.STATE_BEFORE_AFTER_CHILD_CLOSED_OWNER
      + 1);

  /** the process of closing the element has begun */
  private static final int STATE_CLOSE_BEGIN = (HierarchicalFSM.STATE_AFTER_AFTER_CHILD_CLOSED_OWNER
      + 1);

  /** the element is before calling its onClose method */
  private static final int STATE_BEFORE_ON_CLOSE = (HierarchicalFSM.STATE_CLOSE_BEGIN
      + 1);

  /** the element has called its onClose method */
  private static final int STATE_AFTER_ON_CLOSE = (HierarchicalFSM.STATE_BEFORE_ON_CLOSE
      + 1);

  /** the element has called its onChildClose method */
  private static final int STATE_BEFORE_AFTER_CHILD_CLOSED_CHILD = (HierarchicalFSM.STATE_AFTER_ON_CLOSE
      + 1);

  /** the element has called its onClose method */
  private static final int STATE_AFTER_AFTER_CHILD_CLOSED_CHILD = (HierarchicalFSM.STATE_BEFORE_AFTER_CHILD_CLOSED_CHILD
      + 1);

  /** the element is finished */
  private static final int STATE_CLOSED = (HierarchicalFSM.STATE_AFTER_AFTER_CHILD_CLOSED_CHILD
      + 1);
  ...
  protected final void assertNoChildren() {
    ...
    if (fsm != null) {
      ...
      FSM._name(this, sb);
      sb.append(
          " currently should not have any open children, but has child "); //$NON-NLS-1$
      FSM._name(fsm, sb);
      sb.append('.');
  ...
  protected synchronized void onOpen() {
    this.__assertState(
        (this.m_owner == null) ? HierarchicalFSM.STATE_OPEN_BEGIN
    ...
    if ((owner = this.m_owner) != null) {
      synchronized (owner) {
        synchronized (this) {
          ...
          this.__assertState(
              HierarchicalFSM.STATE_AFTER_AFTER_CHILD_OPENED_CHILD);
  ...
  protected synchronized void beforeChildOpens(final HierarchicalFSM child,
    ...
    this.__assertState(
        HierarchicalFSM.STATE_BEFORE_BEFORE_CHILD_OPENS_OWNER);
   ...
   */
  private final boolean __callBeforeChildOpen(
      final HierarchicalFSM element) {
    final MemoryTextOutput sb;
    final boolean hasOtherChildren;
    ...
    this.__assertState(
        HierarchicalFSM.STATE_AFTER_BEFORE_CHILD_OPENS_OWNER_A);
  ...
  protected synchronized void afterChildOpened(final HierarchicalFSM child,
    ...
    this.__assertState(
        HierarchicalFSM.STATE_BEFORE_AFTER_CHILD_OPENED_OWNER);
  ...
  protected synchronized void afterChildClosed(
    ...
    if (child.m_owner == this) {
      this.__assertState(
          HierarchicalFSM.STATE_BEFORE_AFTER_CHILD_CLOSED_OWNER);
    ...
    looper: for (;;) {
      ...
      synchronized (child) {
        ...
        try {
          ...
          this.__assertState(
              HierarchicalFSM.STATE_AFTER_AFTER_CHILD_CLOSED_OWNER);
