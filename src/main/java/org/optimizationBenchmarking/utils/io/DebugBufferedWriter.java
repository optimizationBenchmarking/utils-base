...
public final class DebugBufferedWriter extends BufferedWriter {
    ...
    try {
      ...
      try {
        throw new IOException(
            ((("Error when writing character '" //$NON-NLS-1$
                + ((char) c)) + "' (") + c + ')'), //$NON-NLS-1$
            tt);
    ...
    try {
      ...
      try {
        try {
          ...
          string = ((((((String.valueOf(cbuf) + ' ') + '[') + off) + ',')
              + len) + ']');
        }
    ...
    try {
      ...
      try {
        try {
          ...
          string = ((((((String.valueOf(csq) + ' ') + '[') + start) + ',')
              + end) + ']');
        }
  ...
  public final Writer append(final char c) throws IOException {
    try {
      ...
      try {
        throw new IOException(
            ((("Error when appending character '" //$NON-NLS-1$
                + c) + "' (") + c + ')'), //$NON-NLS-1$
            tt);
      } catch (final IOException ioe) {
        synchronized (this.m_debug) {
