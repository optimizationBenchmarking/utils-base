...
/**
 * A rational number can be expressed as a fraction of two ({@code long})
 * integer numbers {@code a} and {@code b}, i.e., as {@code a/b}.
 */
public class Rational extends BasicNumber
    implements IImmutable, IMathRenderable {
  ...
  public static final Rational valueOf(final double number) {
    ...
    outer: for (;;) {
      ...
      gcd = MathUtils.gcd(mid_up, mid_down);
      if (gcd <= 0L) {
        break outer;
      }
      mid_up /= gcd;
  ...
  public final long longValue(final RoundingMode round) {
    ...
    switch (round) {
      ...
      case HALF_DOWN: {
        return (((fraction << 1) > down) ? ((integer >= 0)
            ? (Rational.__up(integer)) : (Rational.__down(integer)))
            : integer);
      }
      case HALF_EVEN: {
        if ((integer & 1L) != 0L) {
   ...
   */
  public final Rational multiply(final double d) {
    return this.multiply(Rational.valueOf(d));
  }
  ...
  public final Rational multiply(final Rational b) {
    final long a_up, a_down, b_up, b_down, new_up, new_down, up1, up2,
        down1, down2, gcd_a_up_b_down, gcd_a_down_b_up, gcd;
    ...
    gcd_a_up_b_down = MathUtils.gcd(a_up, b_down);
    ...
    gcd_a_down_b_up = MathUtils.gcd(a_down, b_up);
    ...
    gcd = MathUtils.gcd(new_up, new_down);
    if (gcd > 1L) {
  ...
  public final String toString() {
    ...
    if (this.m_down == 0L) {
      return BasicNumber.STATE_NAMES[(this.m_up < 0L)
          ? BasicNumber.STATE_NEGATIVE_INFINITY
  ...
  public final BigDecimal multiply(final BigDecimal b) {
    if (this.m_down != 0L) {
      return b.multiply(BigDecimal.valueOf(this.m_up))
          .divide(BigDecimal.valueOf(this.m_down));
    }
