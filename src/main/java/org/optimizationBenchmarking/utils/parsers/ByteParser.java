...
public abstract class ByteParser extends NumberParser<Byte> {
  ...
  public final void validateDouble(final double d) {
    ...
    if (d != d) {
      throw new IllegalArgumentException(
          d + " (not-a-number) is not a valid byte value."); //$NON-NLS-1$
    }

    if ((d < Byte.MIN_VALUE) || (d > Byte.MAX_VALUE)) {
      throw new IllegalArgumentException(((((d
          + " is not a valid byte value, since it is out of the range ") + //$NON-NLS-1$
    ...
    if (b != d) {
      throw new IllegalArgumentException(
          d + " cannot be converted to a byte without loss of fidelity."); //$NON-NLS-1$
    }
    this.validateByte(b);
  }
