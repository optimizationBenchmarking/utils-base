...
public abstract class FloatParser extends NumberParser<Float> {
            ...
            if (d == (-0d)) {
              ...
              if ((d < (-Float.MAX_VALUE)) || (d > (Float.MAX_VALUE))) {
                throw new IllegalArgumentException(((((d
                    + " is not a valid float value, since it is out of the range [") //$NON-NLS-1$
                    + (-Float.MAX_VALUE)) + ',') + //
                    (Float.MAX_VALUE)) + "]."); //$NON-NLS-1$
              }

              if (((d > (0d)) && (d < Float.MIN_VALUE)) || //
                  ((d < (0d)) && (d > (-Float.MIN_VALUE)))) {
                throw new IllegalArgumentException(((((d
                    + " is not a valid float value, since its scale is too small. Non-zero values must be out of the range (") //$NON-NLS-1$
                    + (-Float.MIN_VALUE)) + ',') + //
                    (Float.MIN_VALUE)) + ")."); //$NON-NLS-1$
              }
  ...
  public final void validateLong(final long l) {
    ...
    if (l != f) {
      throw new IllegalArgumentException(
          l + " cannot be converted to a float without loss of fidelity."); //$NON-NLS-1$
    }
    this.validateFloat(f);
  }
