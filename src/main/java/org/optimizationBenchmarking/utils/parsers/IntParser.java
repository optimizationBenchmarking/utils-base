...
public abstract class IntParser extends NumberParser<Integer> {
   ...
   */
  public void validateInt(final int value)
      throws IllegalArgumentException {
    //
  }
  ...
  public final void validateDouble(final double d) {
    ...
    if (d != d) {
      throw new IllegalArgumentException(
          d + " (not-a-number) is not a valid int value."); //$NON-NLS-1$
    }

    if ((d < Integer.MIN_VALUE) || (d > Integer.MAX_VALUE)) {
      throw new IllegalArgumentException(((((d
          + " is not a valid int value, since it is out of the range ") + //$NON-NLS-1$
    ...
    if (b != d) {
      throw new IllegalArgumentException(
          d + " cannot be converted to a int without loss of fidelity."); //$NON-NLS-1$
    }
    this.validateInt(b);
  }
