...
public abstract class LongParser extends NumberParser<Long> {
  ...
  public final void validateDouble(final double d) {
    ...
    if (d != d) {
      throw new IllegalArgumentException(
          d + " (not-a-number) is not a valid long value."); //$NON-NLS-1$
    }

    if ((d < Long.MIN_VALUE) || (d > Long.MAX_VALUE)) {
      throw new IllegalArgumentException(((((d
          + " is not a valid long value, since it is out of the range ") + //$NON-NLS-1$
    ...
    if (b != d) {
      throw new IllegalArgumentException(
          d + " cannot be converted to a long without loss of fidelity."); //$NON-NLS-1$
    }
    this.validateLong(b);
  }
