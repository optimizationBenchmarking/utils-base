 ...
 * constants.
 */
public class LooseIntParser extends IntParser {

  /** the serial version uid */
  private static final long serialVersionUID = 1L;

  /** the globally shared instance of the integer parser */
  public static final LooseIntParser INSTANCE = new LooseIntParser();
            ...
            if ((base = _PreparedString._getBase(str.charAt(1))) != 0) {
              try {
                retVal = prep.getReturn(
                    java.lang.Integer.parseInt(str.substring(2), base));
                break checker;
              } catch (final Throwable canBeIgnored) {
                // we ignore this exception, as we will throw the original
                // one
  ...
  public final boolean equals(final Object other) {
    final LooseIntParser parser;
    if (other == this) {
      return true;
    }
    if (other instanceof LooseIntParser) {
      parser = ((LooseIntParser) other);
