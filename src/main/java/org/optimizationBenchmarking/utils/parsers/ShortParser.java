...
public abstract class ShortParser extends NumberParser<Short> {
  ...
  public final void validateDouble(final double d) {
    ...
    if (d != d) {
      throw new IllegalArgumentException(
          d + " (not-a-number) is not a valid short value."); //$NON-NLS-1$
    }

    if ((d < Short.MIN_VALUE) || (d > Short.MAX_VALUE)) {
      throw new IllegalArgumentException(((((d
          + " is not a valid short value, since it is out of the range ") + //$NON-NLS-1$
    ...
    if (b != d) {
      throw new IllegalArgumentException(
          d + " cannot be converted to a short without loss of fidelity."); //$NON-NLS-1$
    }
    this.validateShort(b);
  }
