package org.optimizationBenchmarking.utils.text.textOutput;

/**
 * A version of the text output wrapped around an
 * {@link java.lang.StringBuilder}.
 */
final class _StringBuilderWrappedTextOutput
    extends _AppendableWrappedTextOutputBase<StringBuilder> {

  /**
   * Create a wrapped text output
   *
   * @param out
   *          the output
   */
  _StringBuilderWrappedTextOutput(final StringBuilder out) {
    super(out);
  }

  /** {@inheritDoc} */
  @Override
  public final AbstractTextOutput append(final CharSequence csq) {
    this.m_out.append(csq);
    return this;
  }

  /** {@inheritDoc} */
  @Override
  public final AbstractTextOutput append(final CharSequence csq,
      final int start, final int end) {
    this.m_out.append(csq, start, end);
    return this;
  }

  /** {@inheritDoc} */
  @Override
  public final AbstractTextOutput append(final char c) {
    this.m_out.append(c);
    return this;
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final char[] chars, final int start,
      final int end) {
    this.m_out.append(chars, start, (end - start));
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final String s, final int start,
      final int end) {
    this.m_out.append(s, start, end);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final char[] chars) {
    this.m_out.append(chars);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final String s) {
    this.m_out.append(s);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final byte v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final short v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final int v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final long v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final float v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final double v) {
    this.m_out.append(v);
  }

  /** {@inheritDoc} */
  @Override
  public final void append(final boolean v) {
    this.m_out.append(v);
  }
}
