...
import org.optimizationBenchmarking.utils.math.functions.basic.DivideByConstantLong;
import org.optimizationBenchmarking.utils.math.functions.basic.Identity;
import org.optimizationBenchmarking.utils.math.functions.basic.MultiplyWithConstantDouble;
import org.optimizationBenchmarking.utils.math.functions.basic.MultiplyWithConstantLong;
import org.optimizationBenchmarking.utils.math.functions.basic.MultiplyWithConstantRational;
...
public class ConversionMatrixBuilder<E extends Enum<E>> extends FSM {
  ...
  private static final int STATE_COMPILED = (ConversionMatrixBuilder.STATE_COMPILING
      + 1);
   ...
   * <code>from={@link org.optimizationBenchmarking.utils.units.ELength#METER}</code>
   * and
   * <code>to={@link org.optimizationBenchmarking.utils.units.ELength#MILLIMETER}</code>
   * to <code>1000</code> means that
   * <code>1 {@link org.optimizationBenchmarking.utils.units.ELength#M}</code>
   * equals
   * <code>1000 {@link org.optimizationBenchmarking.utils.units.ELength#MM}</code>
   * .
   *
   * @param from
   *          the source unit
  ...
  private final int __setFactor(final int from, final int to,
    ...
    testDouble: if (factor instanceof Double) {
      ...
      if ((old == null) || ((old instanceof Double)
          && (old.toString().length() > use.toString().length()))) {
        this.m_factors[from][to] = use;
        ret = 1;
      }
      if (checkOther) {
        if (this.__setFactor(to, from, Double.valueOf(1d / d),
            false) != 0) {
          ret |= 2;
        }
      }
    ...
    testBD: if (use instanceof BigDecimal) {
      ...
      if ((old == null) || (old instanceof Double)
          || ((old instanceof BigDecimal)
              && (b.toEngineeringString().length() < ((BigDecimal) old)
                  .toEngineeringString().length()))) {
    ...
    if (use instanceof Long) {
      ...
      if (checkOther) {
        if (this.__setFactor(to, from, Rational.valueOf(1L, l),
            false) != 0) {
          ret = 2;
        }
      }
  ...
  private static final Number __mul(final Number a, final Number b) {
    ...
    if (a instanceof Long) {
      ...
      if (b instanceof Rational) {
        ...
        return Double.valueOf(rb.doubleValue() * l);
      }
    ...
    if (a instanceof Rational) {
      ...
      return Double.valueOf(ra.doubleValue() * b.doubleValue());
    }
  ...
  public final UnaryFunction[][] compile() {
    ...
    final int[][] changed;
    final int len;
    final HashMap<Object, UnaryFunction> map;
    int iteration;
    UnaryFunction f, g;
    int i, j, k, change, firstChange, secondChange, changeSum, maxChange,
        limitChange;
    boolean improved;
    Number a, b;
    ...
    for (i = factors.length; (--i) >= 0;) {
      for (j = factors.length; (--j) >= 0;) {
        if (i == j) {
          ...
          f = map.get(a);
          if (f == null) {
            f = ConversionMatrixBuilder._get(a);
            g = map.get(f);
            if (g != null) {
              f = g;
            } else {
  ...
  static final UnaryFunction _get(final Number n) {
    ...
    if (n instanceof Long) {
      ...
      return new MultiplyWithConstantLong(l);
    }
    ...
    if (n instanceof Rational) {
      ...
      if (inv.isInteger()) {
        ...
        return new DivideByConstantLong(l);
      }
      return new MultiplyWithConstantRational(r);
    }
    ...
    if (n != null) {
      ...
      return new MultiplyWithConstantDouble(d);
    }
