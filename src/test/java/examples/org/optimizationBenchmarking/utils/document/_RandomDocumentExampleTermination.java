...
final class _RandomDocumentExampleTermination extends Thread {
  ...
  private static final int TERMINATING = (_RandomDocumentExampleTermination.RUNNING
      + 1);
  /** the example is running but has completed all necessities */
  private static final int ALL_COMPLETED = (_RandomDocumentExampleTermination.TERMINATING
      + 1);
  /** the example is has terminated */
  private static final int TERMINATED = (_RandomDocumentExampleTermination.ALL_COMPLETED
      + 1);
  ...
  final _ERandomDocumentExampleElements _suggest(final Random rand,
    ...
    choose = (((from == null) || (from.length <= 0))
        ? _ERandomDocumentExampleElements.ELEMENTS : from);
            ...
            if (element.m_label != null) {
              ...
              synchronized (labels) {
                ...
                if ((!drop) && ((emergencyFlushLabels > 180) && //
                    ((emergencyFlushLabels
                        % _ERandomDocumentExampleElements.ELEMENTS.length) == //
                        element.ordinal()))) {
