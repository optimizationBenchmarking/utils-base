...
public class GraphicExample extends FileProducerExample<IGraphicDriver> {
  ...
  protected String getMainDocumentNameSuggestionBase() {
    ...
    if ((this.m_size != null)
        && (!(this.m_size.equals(this.getDefaultSize())))) {
      name += '_';
      name += this.m_size.toString();
    }
    if ((this.m_model != null)
        && (!(this.m_model.equals(this.getDefaultColorModel())))) {
      name += '_';
      name += this.m_model.toString();
    }
    if ((this.m_dpi > 0) && (this.m_dpi != this.getDefaultDotsPerInch())) {
      name += '_';
      name += this.m_dpi;
      name += "dpi"; //$NON-NLS-1$
    }
    if ((this.m_quality >= 0d)
        && (this.m_quality != this.getDefaultQuality())) {
      name += '_';
