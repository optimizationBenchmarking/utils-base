...
public class ExampleDocument {
  ...
  private static final ExampleNamespace __createExampleNamespace(
    ...
    loop: for (;;) {
      ...
      return new ExampleNamespace(uri, RandomUtils
          .longToString(RandomUtils.DEFAULT_CHARSET, rand.nextLong()));
    }
  }
  ...
  private static final ExampleElement __createExampleElement(
    ...
    children = ((depth < 100) && (nodes[0] > 0)
        && (force || (rand.nextInt(4) > 0)));
    ...
    return new ExampleElement(namespaces[rand.nextInt(namespaces.length)], //
        RandomUtils.longToString(RandomUtils.DEFAULT_CHARSET,
            rand.nextLong()), //
        ((attrs.size() > 0)
            ? attrs.toArray(new ExampleAttribute[attrs.size()])
            : ExampleDocument.EMPTY_ATTRIBUTES), //
