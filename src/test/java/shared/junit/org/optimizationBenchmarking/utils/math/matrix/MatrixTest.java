package shared.junit.org.optimizationBenchmarking.utils.math.matrix;
        ...
        if (copy.isIntegerMatrix() && cur.isIntegerMatrix()) {
          ...
          if (cur.isIntegerMatrix() == copy.isIntegerMatrix()) {
            for (i = m; (--i) >= 0;) {
              for (j = n; (--j) >= 0;) {
                Assert.assertTrue(
                    cur.getDouble(i, j) == copy.getDouble(i, j));
              }
            }
          } else {
            a = b = true;
  ...
  private final void __testSelected(final IMatrix orig, final IMatrix sel,
    ...
    if (depth > 0) {
      outer: for (k = 3; (--k) >= 0;) {
        ...
        switch (k) {
          case 0: {
            this.__testSelected(orig.transpose(), sel.transpose(), selCols,
                selRows, rand, (depth - 1));
            continue outer;
          }
