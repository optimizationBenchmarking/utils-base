...
/**
 * A class which can generate random text in the style of
 * <a href="http://en.wikipedia.org/wiki/Lorem_ipsum">Lorem Ipsum</a>. Such
...
public final class LoremIpsum {
  ...
  private static final int MAX_WORD_LENGTH_P1 = (LoremIpsum.TEXT[LoremIpsum.TEXT.length
      - 1].length() + 1);

  /**
   * Append some random
   * <a href="http://en.wikipedia.org/wiki/Lorem_ipsum">Lorem Ipsum</a>,
   * i.e., a randomized filler text.
   *
   * @param out
   *          the output
   * @param rand
  ...
  public static final void appendLoremIpsum(final ITextOutput out,
    ...
    int count, lastLength, curLength, sentencePartLength,
        sentencePartCount;
  ...
  public static final String loremIpsum(final Random rand,
    ...
    if ((maxLength > 0) && (maxLength < 1000000)) {
      out = new MemoryTextOutput(
          maxLength * LoremIpsum.MAX_WORD_LENGTH_P1);
    } else {
      out = new MemoryTextOutput();
    }
