package test.junit.org.optimizationBenchmarking.utils.units;
...
public class ELengthTest {
  ...
  private static final TestCase[] TESTS = {
      ...
      new TestCase(ELength.LI_CHINESE, ELength.HU_CHINESE, 1L,
          1500000000L),
      ...
      new TestCase(ELength.DIDOT, ELength.MILLIMETER, 1000L, 376l),
      new TestCase(ELength.INCH, ELength.PICA, 1L, 6L), new TestCase(
          ELength.POINT, ELength.UM, 1000000000L, 352777777777L, false),

      new TestCase(ELength.MM, ELength.POINT, 100000000000000L,
          283464566929133L, false),

      new TestCase(ELength.MILE_INTERNATIONAL, ELength.KILOMETER, 1000000L,
          1609344L),
  ...
  private static final boolean __isDifferent(final double should,
      final double is) {
    return ((should != is)
        && (Math.abs(should - is) > (1e-9 * Math.abs(should))));
  }
  ...
  private static final void __testConvert(final ELength from,
    ...
    calc = false;
    try {
      res = from.convertToAsLong(fromValue, to);
      calc = true;
    ...
    if (res != toValue) {
      if (strict || ELengthTest.__isDifferent(toValue, res)) {
        throw new AssertionFailedError(("Failed to convert '" + //$NON-NLS-1$
            ...
            " " + to + //$NON-NLS-1$
            (calc
                ? ("' - calculated '" + res + //$NON-NLS-1$
                    "' instead.")//$NON-NLS-1$
                : ("got an error instead."))) //$NON-NLS-1$
            + " The conversion function is "//$NON-NLS-1$
            + from.getConversionFunction(to));
      }
    }

    calc = false;
    try {
      re = from.convertToAsDouble(((double) fromValue), to);
      calc = true;
    } catch (final Throwable xxx) {
      re = Double.NaN;
    }
